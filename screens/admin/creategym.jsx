...
export class AddGymScreen extends React.Component {
  ...
  handleCreate = () => {
    ...
    this.props.gymServices
      .addGym(name, openTimeText, closeTimeText, longitude, latitude)
      .then((r) => {
          ...
          <View>
            <Form>
              ...
              <TouchableOpacity
                style={styles.button}
                onPress={() => {
                  this.setState({ isVisible: true, currentSelection: "open" });
                }}
              >
                <Text style={styles.text}>
                  {" "}
                  {this.state.openTimeText.length == 0
                    ? "Select open time"
                    : this.state.openTimeText}{" "}
                </Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                style={styles.button}
                onPress={() => {
                  this.setState({ isVisible: true, currentSelection: "close" });
                }}
              >
                <Text style={styles.text}>
                  {" "}
                  {this.state.closeTimeText.length == 0
                    ? "Enter close time"
                    : this.state.closeTimeText}{" "}
                </Text>
              </TouchableOpacity>

              <Item regular style={(styles.button, { marginTop: 15 })}>
                <Input placeholder="Longitude" value={this.state.latitude} />
              </Item>
              <Item regular>
                <Input placeholder="Latitude" value={this.state.longitude} />
              </Item>
            </Form>
          </View>
